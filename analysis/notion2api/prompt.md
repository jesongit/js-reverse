在工作目录：analysis/notion2api 下 使用 js-reverse 帮我实现把网页对话转成 Open API 的功能
...
邮箱可以使用 posase当前时间戳@example.test，用户名使用邮箱前缀
...
工作目录：analysis/notion2api
使用 js-reverse 这个 skill，结合现有内容，进行下述优化
1. 对话改完真实流式而不是伪流式
2. 分析更多关于AI相关接口并详细记录到分析报告中，方便后续开发，包括但不限于获取账号剩余额度、调用次数账号信息等